...
async function main() {
  ...
  if (!salesUser) throw new Error('Seed: sales_user not created');
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    const expectedRevenue = i % 5 === 0 ? (i * 100_000_000) : null;

    const influenceRevenue = (i % 4) + 1 > 3 ? 3 : (i % 4);
    const influenceCustomer = i % 3 === 0 ? 2 : 1;
    const influenceCustomization = i % 2 === 0 ? 2 : 1;
    const influenceStrategy = i % 5 === 0 ? 2 : 1;
    const influenceTender = i % 4 === 0 ? 1 : 0;
    const influenceTotal = influenceRevenue + influenceCustomer + influenceCustomization + influenceStrategy + influenceTender;
